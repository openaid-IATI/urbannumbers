...
function UnhabitatOipaCompareFilters(){
	this.get_url = function(selection, parameters_set) {
		...
		if (parameters_set){
			var cururl = search_url + "indicator-filter-options/?format=json&categories__in=Population,City%20prosperity,Slumd%20wellers,Streets,Transport,Health,Resilience,Education,Crime,Land%20area&adm_division__in=city" + parameters_set;
		} else {
			var cururl = search_url + "indicator-filter-options/?format=json&categories__in=Population,City%20prosperity,Slumd%20wellers,Streets,Transport,Health,Resilience,Education,Crime,Land%20area&adm_division__in=city&indicators__in=" + get_parameters_from_selection(this.selection.indicators);
		}
...
function UnhabitatOipaIndicatorFilters() {
    ...
    this.get_url = function(selection, parameters_set){
        // get url from filter selection object
        if (parameters_set){
            var cururl = search_url + "indicator-filter-options/?format=json&categories__in=Population,City%20prosperity,Slumd%20wellers,Streets,Transport,Health,Resilience,Education,Crime,Land%20area" + parameters_set;
        } else {
